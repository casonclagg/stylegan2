...
def generate_image_from_dumb_pickle(latent_vector):
    images = Gs.run(latent_vector, **Gs_kwargs)
    return images
...
def make_latent_interp_animation(code1, code2, num_interps, index):
    ...
    for alpha in tqdm(amounts):
        ...
        # images = generate_image_from_dumb_pickle(interpolated_latent_code)
        PIL.Image.fromarray(images[0], 'RGB').save(f'results/pk3-out/{count:06}.png')
        count += 1
...
def get_final_latents():
    ...
    latent_files = [x for x in Path('results/pk3').iterdir()]
    latent_files.sort()
    all_final_latents = []
...
def main():
    files = get_final_latents()
    for index, obj in enumerate(files):
        if index > 0:
            end = obj
            start = files[index - 1]
            make_latent_interp_animation(end,start, 150, index-1)
