...
def make_latent_interp_animation(z_index, z, num_interps, og_z):
    ...
    for alpha in tqdm(amounts):
        val = og_value + math.sin(alpha) * 10
        for x in range(18):
            z[0][x][z_index] = val
        images = generate_image_from_pickle(z)
        PIL.Image.fromarray(images[0], 'RGB').save((f'results/zs-fixed/{count:06}.png'))
        count += 1
        z[0][0][z_index] = og_z[z_index]
        
# python generate-zs.py "results/pickles/cason-2.pkl" 60
def main():
    ...
    for z_index in range(512):
        ...
        make_latent_interp_animation(z_index, start_z, numberOfFrames, og_z)
